...
const API_URL = "http://localhost:3003/customers";
...
export const createCustomer = async (customerData, token) => {
  const res = await axios.post(API_URL, customerData, getConfig(token));
  return res.data;
};
...
export const updateCustomer = async (id, customerData, token) => {
  const res = await axios.put(`${API_URL}/${id}`, customerData, getConfig(token));
  return res.data;
};
